...
| D7 | ชื่อนายจ้าง "จงสถิตย์" | **ยืนยันแล้วโดยเจ้าของโปรไฟล์ (Tae) — ใช้ชื่อจริงตามที่เขียนไว้ใน `docs/PROFILE.md` ได้** ไม่ต้องเปลี่ยนเป็นคำเลี่ยงทั่วไป | เจ้าของโปรไฟล์ยืนยันด้วยตัวเองว่าเผยแพร่ชื่อบริษัทบนเว็บสาธารณะได้ (2026-09-25) — ปิดความเสี่ยงที่ Devil's Advocate ยกไว้ | Devil (ยก) → Tae (ยืนยัน) |
...
| D9 | `/api/contact` orphan endpoint | ลบ `src/pages/api/contact.ts` ออก (ปิด public write endpoint ไม่ให้ deploy) แต่คงฟังก์ชัน `insertContact`/`ContactMessage` ไว้ใน `src/lib/db.ts` เพราะ `tests/labs/lab05-api.test.ts` เรียกใช้ตรงจาก `db.ts` (ไม่ผ่าน route) และห้ามแก้ test ตามกติกาคอร์ส | หลัง D2 ไม่มีฟอร์มเรียกใช้ endpoint นี้อีกต่อไป (ยืนยันด้วย grep `api/contact` ทั่ว `src/`/`tests/`/`playwright/` ไม่เจอผู้เรียกใช้อื่น) การปล่อยให้ public write endpoint ที่ไม่มีผู้เรียกใช้เปิดอยู่เป็นความเสี่ยงเปล่าประโยชน์ ตัดออกตรง D2 โดยไม่ทำให้ `test:labs` แดง | OpenCode (วิจัย+เสนอ ใน Lab 05b swarm) → Claude (ตัดสินใจปิด) |
