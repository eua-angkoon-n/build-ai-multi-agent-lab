...
- **Lab 06 + 07 เสร็จ — PR #22 (Lab 06) merge แล้ว, cross-model review comment โพสต์บน PR #17 แล้ว** — Lab 07: OpenCode รีวิว PR #17 (diff), เขียน `docs/review-opencode.md` (Must 2/Should 3/Nit 4) → Claude ตอบใน `docs/review-claude-rebuttal.md` (ยืนยัน M2/S2 ปิดแล้วจริง, แก้ S1 + S3 สด ๆ ในรอบนี้) → OpenCode รอบสองปิด Must ทั้งคู่ (`## Round 2 — close`) → โพสต์รวมเป็น PR comment บน #17 · เปิด L15 (test blind spot เชิงโครงสร้าง, ไม่บล็อก) · เหลือ L10 (ปิด D5 formal), L6/L13/L14/L15 (ไม่บล็อก) ก่อนไป Lab 08

## Done (วันนี้ 2026-09-25 บนเครื่องนี้)

- **Lab 07 (Cross-model review) เสร็จ:** เลือก PR #17 (Frontend/Claude เขียน) ให้ OpenCode รีวิว ตามแผน "PR ที่ Claude เขียน → OpenCode รีวิว" — `opencode run` เขียน `docs/review-opencode.md` (อ่านของจริงในไฟล์ปัจจุบัน ไม่ใช่แค่ diff เฉย ๆ, เจอว่า M2/S2 ที่ตัวเองยกมาปิดไปแล้วจริงผ่าน PR #18/#20) → Claude ตอบใน `docs/review-claude-rebuttal.md`: ยืนยัน M1 ยังเปิด (เป็น human decision, ไม่ใช่การปัดตก), ยืนยัน M2/S2 ปิดแล้วจริง, **แก้ S1 จริง** (`tests/public-site.test.ts` COURSE_LEAK_PATTERN ไม่เคยจับคำว่า "course" เลย — ขยาย regex), **แก้ S3 จริง** (เพิ่ม `normalizeGithub()` ใน `profile.ts` กัน URL พังเงียบถ้า PROFILE.md เขียน username เปล่า) · รอบสอง: `opencode run` อ่าน rebuttal แล้วตัดสิน M1=accept(valid rebuttal), M2=accept(fixed) พร้อมตรวจโค้ดจริงซ้ำก่อนตัดสิน (ไม่เชื่อคำอ้างเฉย ๆ) → append `## Round 2 — close` ใน `docs/review-opencode.md` · โพสต์รวมเป็น PR comment บน #17 จริง (ไม่ใช่แค่ในแชท) · เปิด L15 สำหรับช่องว่างเชิงโครงสร้างที่เหลือ (frontmatter-scan) · `npm test`/`test:labs`/`build` เขียวหมดหลังแก้
